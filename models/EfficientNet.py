...
class MBConvBlock(nn.Module):
    ...
    def __init__(self, block_args, global_params, image_size=None):
        ...
        output_channels = self._block_args.input_filters * self._block_args.expand_ratio  # number of output channels
        '''
        blocks_args = [
        ...
        if self._block_args.expand_ratio != 1:
            self._expand_conv = nn.Conv2d(in_channels=input_channels, out_channels=output_channels, kernel_size=1, bias=False, padding=0)
            self._bn0 = nn.BatchNorm2d(num_features=output_channels, momentum=self._bn_momentum, eps=self._bn_eps)

        # Depthwise
        kernel_size = self._block_args.kernel_size
        stride = self._block_args.stride
        self._depthwise_conv = nn.Conv2d(
            in_channels=output_channels, out_channels=output_channels, groups=output_channels,  # groups makes it depthwise
            kernel_size=kernel_size, stride=stride, bias=False, padding=(1 if kernel_size == 3 else 2))
        self._bn1 = nn.BatchNorm2d(num_features=output_channels, momentum=self._bn_momentum, eps=self._bn_eps)

        # Squeeze and Excitation
        if self.has_se:
            squeezed_channels = max(1, int(input_channels * self._block_args.se_ratio)) # 0.25 basically
            self._se_reduce = nn.Conv2d(in_channels=output_channels, out_channels=squeezed_channels, kernel_size=1)
            self._se_expand = nn.Conv2d(in_channels=squeezed_channels, out_channels=output_channels, kernel_size=1)

        # Pointwise
        final_oup = self._block_args.output_filters
        self._project_conv = nn.Conv2d(in_channels=output_channels, out_channels=final_oup, kernel_size=1, bias=False)
        self._bn2 = nn.BatchNorm2d(num_features=final_oup, momentum=self._bn_momentum, eps=self._bn_eps)
